...
# control zone
chapter_ranges = [x for x in range(1, 16)]+[19]
...
df = pd.merge(df, df_single_record[['eid', 'diseases_within_window_all_icd_first_3', 'diseases_within_window_icd_parent_coding', ]], how='left', left_on='eid', right_on='eid')
...
# ----------------------------------------------------------
df = pd.read_pickle(params.final_data_path/ 'UKB_wave_0_final_non_standardised.pkl')
df_subset = df[['disease_count',disease_column, '31', '21022']].copy()


# self-repeating occurrence
# diseases related vars ['disease_count', 'chapter_1-22']

df_to_plot = df[['31', '21022']+[f'chapter_{x}' for x in range(1, 23)]]

df_to_plot = df_to_plot.pivot_table(index=['31','21022'],values=[f'chapter_{x}' for x in range(1, 23)],aggfunc=np.mean)
...
plt.savefig(params.current_path/'plot/disease_co_occurrence_within_diseases_by_chapter.pdf')
...
plt.savefig(params.current_path/f'plot/disease_co_occurrence_between_diseases_by_{level}.pdf')
