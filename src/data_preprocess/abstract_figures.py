...
# =================================================================================================
# diseases by education
# =================================================================================================
df_subset = df[[f'{level}_disease_count',disease_columns[1], '21022','31','10722']].copy()
df_subset[f'10722'].replace({3:1,2:1, list(df_subset['10722'].unique())[1]:1, 4:2,1:2,5:3},inplace=True)
df_subset=df_subset[(df_subset['21022']>40) & (df_subset['21022']<70)]

df_subset.drop(df_subset.loc[df_subset['chronic_cate_disease_count']==0].index, inplace=True)

for chapter in df_subset[disease_columns[1]].explode().dropna().unique():
    df_subset[chapter] = [x.count(chapter) if str(x) not in params.nan_str else 0 for x in df_subset[disease_columns[1]]]
...
df_subset.rename(columns=phe_cate_dict,inplace=True)


# Plot the mean values of each chapter column based on the sorted '10722' column


# set 10722 as categorical
colors = ['blue', 'green', 'red']
df_sorted = df_subset.loc[df_subset['31'] == 0].sort_values(by='10722')
from scipy import stats
for chapter in ['genitourinary', 'endocrine/metabolic',
       'circulatory system', 'pregnancy complications', 'musculoskeletal',
       'digestive', 'neoplasms', 'neurological', 'dermatologic',
       'congenital anomalies']:
    plt.figure(figsize=(7, 8))
    # groups = df_sorted[['10722', chapter,'21022']].groupby(['21022','10722'], as_index=False).mean()
    for i in range(1,4):
        temp = df_sorted.loc[df_sorted['10722']==i]


        slope, intercept, r_value, p_value, std_err = stats.linregress(temp['21022'], temp[chapter])
        line = slope * temp['21022'] + intercept

        # Plot the fitted line
        plt.plot(temp['21022'], line, color=colors[i-1], label=i)

    # Set plot labels and title
    plt.xlabel('10722 (sorted)')
